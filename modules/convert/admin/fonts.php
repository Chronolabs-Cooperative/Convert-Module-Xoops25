 ...
 */
	...
	if ($start>0 && count($fonts)==0 && $total > 0)
	{
		...
		if ($start<0)
			$start = 0;
		header("Location: " . XOOPS_URL . "/modules/" . _MD_CONVERT_MODULE_DIRNAME . "/admin/fonts.php?start=$start&limit=$limit");
		exit(0);
	}
	
	if ($start == 0 && count($fonts) == 0)
	{
		redirect_header(XOOPS_URL . "/modules/" . _MD_CONVERT_MODULE_DIRNAME . "/admin/index.php", 4, _ERR_CONVERT_ADMIN_NOFONTS);
		exit(0);
	}
	...
	foreach($fonts as $font)
		$GLOBALS['xoopsTpl']->append('fonts', $font->getHistoryTile());
	xoops_load('XoopsPageNav');
	$nav = new XoopsPageNav($total, $limit, $start, 'start', '&limit='.$limit);
	$GLOBALS['xoopsTpl']->assign('pagenav', $nav->renderNav(5));
	echo $GLOBALS['xoopsTpl']->display(dirname(__DIR__) . '/templates/admin/convert_fonts.html');
	xoops_cp_footer();
	exit(0);
