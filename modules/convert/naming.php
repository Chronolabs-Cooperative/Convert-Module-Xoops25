 ...
 */
	...
	if ($GLOBALS['convertConfigsList']['htaccess']) {
		if (!strpos($font->getNamingCueURL('referee'), $_SERVER['REQUEST_URI']) && !strpos($font->getNamingCueURL('barcode'), $_SERVER['REQUEST_URI'])) {
			header('Location: ' . $font->getNamingCueURL('referee'));
			exit(0);
		}
	}
	...
	{
		sleep(mt_rand(1,9));
		$image = array('data' => $font->getNamingCue());
		XoopsCache::write(md5(__FILE__.$_GET['id']), $image, 8444);
